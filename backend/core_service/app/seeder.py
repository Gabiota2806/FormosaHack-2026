"""
Seeder de Datos Reales de Ciberestafas y Canales Oficiales para CiberGuardián.
Puebla PostgreSQL con 10 amenazas reales y 5 canales verificados de Formosa.
...
INCIDENTS = [
    ...
    },
    {
        ...
        "impersonated_entity": "REFSA Electricidad",
        "attack_vector": "WHATSAPP",
...
def run_seed(db: Session) -> dict:
    """
    Ejecuta la siembra de forma idempotente: no duplica canales (por entity_name)
    ni incidentes (por title). Retorna conteos de creados y omitidos.
    """
    result = {
        "created": {"channels": 0, "incidents": 0},
        "skipped": {"channels": 0, "incidents": 0},
    }

    for channel in OFFICIAL_CHANNELS:
        existing = db.query(OfficialChannel).filter_by(entity_name=channel["entity_name"]).first()
        if existing:
            result["skipped"]["channels"] += 1
        else:
            db.add(OfficialChannel(**channel))
            result["created"]["channels"] += 1

    for incident in INCIDENTS:
        existing = db.query(IncidentReport).filter_by(title=incident["title"]).first()
        if existing:
            result["skipped"]["incidents"] += 1
        else:
            db.add(IncidentReport(**incident))
            result["created"]["incidents"] += 1
...
def main() -> None:
    ...
    try:
        result = run_seed(db)
        for key in ("channels", "incidents"):
            print(f"  ✅ {key}: {result['created'][key]} creados, {result['skipped'][key]} omitidos (ya existían)")
        print("🎉 ¡Base de datos poblada exitosamente con datos realistas para la evaluación!")
    except Exception as exc:
        db.rollback()
        print(f"❌ Error al sembrar datos: {exc}")
        raise
    finally:
        db.close()
